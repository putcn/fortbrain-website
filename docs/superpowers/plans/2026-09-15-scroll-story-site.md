...
- [ ] **Step 1: 写脚本**：读 `~/Fortbrain/packs/mxipos/app/geo/320300.json`；Douglas-Peucker（自实现，容差 0.0025°）简化所有环和折线；roads 只留 `motorway`/`trunk`；water 面按鞋带公式面积 < 2e-4 平方度丢掉；坐标保留 4 位小数；输出紧凑 JSON。
- [ ] **Step 2: 跑脚本**：`python3 scripts/build-geo.py`，检查 `ls -la data/xuzhou.json` < 300KB，`python3 -c` 确认 districts 数为 10。
- [ ] **Step 3: Commit** `feat: build simplified Xuzhou basemap`
...
- [ ] **Step 1: 写失败测试**：投影中心为原点；`placeStores` 20 个点两两距离 ≥ cellH×0.8；`pickFeatured` 返回 n 个、两两距离大于随机取 n 个的平均。
- [ ] **Step 2: `node --test tests/`** 失败。
- [ ] **Step 3: 从大屏 `ui/src/display/geo.js` 移植并加 `pickFeatured`**。
- [ ] **Step 4: 测试通过。**
- [ ] **Step 5: Commit** `feat: geo math with featured-store picking`
...
- [ ] **Step 1: 测试** zh/en 两表 key 集合完全一致；`detectLang` 在 `zh-CN` 返回 `zh`，`en-US` 返回 `en`。
- [ ] **Step 2: 失败 → 实现 → 通过。**
- [ ] **Step 3: Commit** `feat: bilingual copy table`
...
- [ ] **Step 1: index.html**：`<canvas id="gl">` 固定；`<main>` 下 9 个 `<section data-stage=N>`，每段 `100svh`，内部 `.copy`（label/title/body 带 `data-i18n`）；右上 `#langBtn`；结尾段 `#contact`。
- [ ] **Step 2: site.css**：深蓝底 `#050b18`、字体栈、`.copy` 左栏 max 560 / 手机全宽底对齐 + 渐变底；`.on` 时淡入上移；reduced-motion 关过渡。
- [ ] **Step 3: fallback.js** 按 §8.1 实现（离屏 0.5×、3–4 条带、竖向光丝、两层噪声、lighter 叠加）。
...
- [ ] **Step 1: 删掉旧站残留；`README` 一段说明如何本地预览（`python3 -m http.server`）与重生成底图。**
- [ ] **Step 2: 全流程走查：桌面 Chrome/Safari、iOS Safari 模拟、断网底图失败、reduced-motion、`?nogl=1`。**
- [ ] **Step 3: Commit** `chore: finalize scroll-story site`
