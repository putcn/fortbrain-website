...
test('there are 11 stages; every stage is pinned for a screen, the layers stage for two', () => {
...
test('progressToStage follows the DOM: p spans (total − 1) screens of scroll', () => {
  ...
  const scrollable = STAGES.reduce((a, b) => a + b, 0) - 1   // 22 screens
  const at = (screens) => progressToStage(screens / scrollable)
  // section tops: 0,2,4,7,9,11,13,15,17,19,21 screens
  ...
  // finale: its top reaches the viewport at 21 screens, and the one remaining screen is u 0→1
  const s10 = at(21); assert.equal(s10.k, 10); assert.ok(Math.abs(s10.u) < 1e-9)
  const m10 = at(21.5); assert.equal(m10.k, 10); assert.ok(Math.abs(m10.u - 0.5) < 1e-9)
