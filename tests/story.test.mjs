...
test('there are 12 stages; the hero is one unpinned screen, every other stage is pinned for a screen, the layers stage for two', () => {
...
test('progressToStage follows the DOM: p spans (total − 1) screens of scroll', () => {
  ...
  assert.deepEqual(progressToStage(1), { k: 11, u: 1 })
  const scrollable = STAGES.reduce((a, b) => a + b, 0) - 1   // 23 screens
  const at = (screens) => progressToStage(screens / scrollable)
  // section tops: 0,1,3,6,8,10,12,14,16,18,20,22 screens
  ...
  const h10 = at(21); assert.equal(h10.k, 10); assert.ok(Math.abs(h10.u - 0.5) < 1e-9)
  // finale: its top reaches the viewport at 22 screens, and the one remaining screen is u 0→1
  const s11 = at(22); assert.equal(s11.k, 11); assert.ok(Math.abs(s11.u) < 1e-9)
  const m11 = at(22.5); assert.equal(m11.k, 11); assert.ok(Math.abs(m11.u - 0.5) < 1e-9)
  assert.deepEqual(progressToStage(-1), { k: 0, u: 0 })
  assert.deepEqual(progressToStage(2), { k: 11, u: 1 })
})
...
test('blend holds the stage until half way, then eases to the next stage', () => {
  ...
  assert.deepEqual(blend(11, 0.9), { a: 11, b: 11, t: 1 })
  let prev = -1
